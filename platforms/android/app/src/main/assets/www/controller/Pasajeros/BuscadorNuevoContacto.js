myApp.onPageInit('NuevoContacto', function (page) {
    document.getElementById('buscarNuevoContacto').focus();
	...
	$("#buscarNuevoContacto").on('keyup', function(){
        var value = $(this).val();
        $.ajax({
	        type: 'POST', 
	        url:  window.server + 'buscadorNuevoContacto.php',
	        data:   ({
	                    match: value,
	        ...
	        success: function(data){
	            ...
	            if(data != 'error'){
	                ...
	                $.each(obj, function(i,contacto){
	                    html = html + "<li>";
						    html = html + "<a href='view/Pasajeros/detail.html' onclick='asignaId("+'"'+contacto.id+'"'+");' class='item-link item-content' id='cl'> ";
	                ...
	                $("#resultsNuevoContacto").html(html);
	      //           if (obj.length == 1) {
