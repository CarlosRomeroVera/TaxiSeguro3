myApp.onPageInit('ayuda', function (page) {
	// alert('ok');
    document.getElementById('buscarContactoPeligro').focus();
	...
	$("#buscarContactoPeligro").on('keyup', function(){
        var value = $(this).val();
        $.ajax({
	        type: 'POST', 
	        url:  window.server + 'buscadorContactoPeligro.php',
	        data:   ({
	                    match: value,
	        ...
	        success: function(data){
	            ...
	            if(data != 'error'){
	                ...
	                $.each(obj, function(i,contacto){
	                    html = html + "<li>";
						    html = html + "<a href='view/Pasajeros/detailContactoPeligro.html' onclick='asignaId("+'"'+contacto.id+'"'+");' class='item-link item-content' id='cl'> ";
	                ...
	                $("#resultsContactoPeligro").html(html);
	      //           if (obj.length == 1) {
